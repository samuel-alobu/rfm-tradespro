...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'send') {
      ...
      if (!user.emailVerified) {
        ...
        if (!emailResult.success) {
          console.error('Failed to send account verification email:', emailResult.error);
      ...
      const emailResult = await sendTwoFactorEmail(user.email, verificationCode);
    ...
    if (action === 'verify') {
      ...
      user.twoFactorCode = undefined;
      user.twoFactorExpires = undefined;
      user.twoFactorVerifiedUntil = new Date(Date.now() + 5 * 60 * 1000);
      await user.save();
